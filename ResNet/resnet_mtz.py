...
class ResNet_MTZ:
    ...
    def merge_initial_tasks(self, task_A, task_B):
        ...
        for layer_name in layer_list:
            ...
            self.build()
            self.test_merged()
            self.joint_finetune(steps=20 + layer_count * 7)
            layer_count += 1
            self.test_merged()
    ...
    def merge_weights_at_layer_add(self, layer_name, task_name):
        ...
        self.weight_dict['shared'][layer_name]['weights'] = np.float32(
            merged_vector[:, :, :, merge_list[:, 0], merge_list[:, 1]])

        self.weight_dict[task_name][layer_name]['weights'] = np.float32(
            self.weight_dict[task_name][layer_name]['weights'][:, :, :, mask])
    ...
    def merge_weights_at_layer(self, layer_name, task_A, task_B, merge_degree=0.9):
        ...
        self.weight_dict[task_A][layer_name]['weights'] = np.float32(
            self.weight_dict[task_A][layer_name]['weights'][:, :, :, mask_A])
        self.weight_dict[task_B][layer_name]['weights'] = np.float32(
            self.weight_dict[task_B][layer_name]['weights'][:, :, :, mask_B])
    ...
    def generate_merge_list_conv_add(self, task_name, layer_name):
        ...
        for i in range(len(self.merged_task_list[:-1])):
            if i == 0:
                ...
                hessian_inverse_A += np.linalg.pinv(
                    self.calculate_hessian_conv_tf(self.merged_task_list[i], layer_name))
        ...
        merged_vector = np.zeros(
            (weights_A.shape[0], weights_A.shape[0], weights_A.shape[2], weights_A.shape[3], weights_B.shape[3]))
    ...
    def construct_model(self, task_name):
        ...
        for i in range(self.n_group):
            for j in range(self.n_blocks_per_group):
                ...
                y, self.layer_input[task_name][block_name + '/conv_2'] = self.res_block(y, block_name, task_name,
    ...
    def conv_layer_basic(self, bottom, layer_name, task_name, stride=1):
        ...
        with tf.variable_scope(task_name + '/' + layer_name):
            bn = tf.layers.batch_normalization(conv, momentum=0.1, epsilon=1e-05, training=self.training,
    ...
    def bn_layer(self, bottom, layer_name, task_name):
        ...
        with tf.variable_scope(task_name + '/' + layer_name):
            bn = tf.layers.batch_normalization(bottom, momentum=0.1, epsilon=1e-05, training=self.training,
                                               beta_initializer=beta, scale=False, moving_mean_initializer=mean,
                                               moving_variance_initializer=variance,
                                               beta_regularizer=self.regularizer_conv)
    ...
    def get_conv_filter_bn(self, task_name, layer_name):
        with tf.variable_scope(task_name + '/' + layer_name):
            weights = tf.get_variable(name="weights", initializer=self.weight_dict[task_name][layer_name]['weights'],
                                      regularizer=self.regularizer_conv)
    ...
    def get_conv_filter_bn_specific(self, task_name, layer_name):
        with tf.variable_scope(task_name + '/' + layer_name):
            ...
            mean = tf.constant_initializer(self.weight_dict[task_name][layer_name]['mean'])
            variance = tf.constant_initializer(self.weight_dict[task_name][layer_name]['variance'])
            permutation = tf.get_variable(name='permutation',
                                          initializer=self.weight_dict[task_name][layer_name]['permutation'],
                                          trainable=False)
    ...
    def get_conv_filter_shared(self, layer_name):
        with tf.variable_scope('shared/' + layer_name, reuse=tf.AUTO_REUSE):
            weights = tf.get_variable(name="weights", initializer=self.weight_dict['shared'][layer_name]['weights'],
                                      regularizer=self.regularizer_conv)
        return weights
    ...
    def get_fc_param(self, task_name, layer_name):
        with tf.variable_scope(task_name + '/' + layer_name):
            weights = tf.get_variable(name="weights", initializer=self.weight_dict[task_name][layer_name]['weights'],
                                      regularizer=self.regularizer_fc)
            biases = tf.get_variable(name="biases", initializer=self.weight_dict[task_name][layer_name]['biases'],
                                     regularizer=self.regularizer_fc)
    ...
    def joint_finetune(self, steps):
        ...
        for i in range(steps):
            _, loss = self.sess.run([self.opt_op, self.loss], feed_dict={'training:0': True},
                                    options=tf.RunOptions(report_tensor_allocations_upon_oom=True))

            total_loss += np.sum(list(loss.values()))
            #            print('{:d}'.format(i))
            if (i + 1) % 5 == 0:
                print('\rbatch={:d}/{:d},curr_loss={:f},used_time:{:.2f}s'.format(i + 1, steps, total_loss / i + 1,
                                                                                  time.time() - time_last), end=' ')
                time_last = time.time()
    ...
    def test(self, task_name):
        ...
        try:
            while True:
                loss_batch, accuracy_batch = self.sess.run([self.loss[task_name], self.accuracy[task_name]],
                                                           feed_dict={'training:0': False})
                total_loss += loss_batch
                total_correct_preds += accuracy_batch
                n_batches += 1
        ...
        print('Testing task {:s}, val_acc={:%}, val_loss={:f}'.format(task_name,
                                                                      total_correct_preds / self.n_samples_val[
                                                                          task_name], total_loss / n_batches))
...
if __name__ == '__main__':
    ...
    for task_name in task_list:
        ...
        model.save_weight('model_weights/' + save_path)
